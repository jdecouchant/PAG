...
class ReceiverTh extends Thread implements Runnable {
    ...
    // Constructor
    public ReceiverTh(int listeningPort, LinkedBlockingQueue<Message> inMessages,
            int selfId) {
        this.mustStop = false;
        this.selfId = selfId;
    ...
    public void run() {
        ...
        try {
            ...
            while (!mustStop) {
                ...
                while (it.hasNext()) {
                    ...
                    if (key.isAcceptable()) {
                        ...
                        if (receiveData == null) {
                            System.err.println("RECV ERROR: receiveData is null!!!");
                            //System.exit(-1);
                        } else {
                        	...
                        	downloadSize += rs;
                            if (receiveMessage.messageType != MessageType.EMPTY_UPDATES && receiveMessage.messageType != MessageType.EMPTY_LOG) {
            ...
            System.err.println("Node " + selfId + ": Problem with port " + listeningPort);
            e.printStackTrace();
            System.exit(-1);
        }
    }
