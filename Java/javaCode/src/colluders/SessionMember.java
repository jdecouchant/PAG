...
public class SessionMember {
	...
	//private final int UPDATE_LEN; // Length of one update in bytes
	...
	public SessionMember (short selfId, int listeningPort, int RTE, int FANOUT_CLI, int FANOUT_SVR, int UPDATE_LEN, ArrayList<Node> nodesList, int sessionDuration) {
		this.RTE = RTE;
		this.SESSION_DURATION= sessionDuration; 
		if (selfId != 0) // The node is a peer
			this.peer = new Peer(selfId, listeningPort, nodesList, FANOUT_CLI, RTE, UPDATES_PER_ROUND, UPDATE_LEN);
		else // The node is the broadcaster
			this.server = new Source(listeningPort, nodesList, FANOUT_SVR, UPDATES_PER_ROUND, UPDATE_LEN);   
	}
		...
		if (selfId != 0) { // The node is a peer
			try {
				...
				while (System.currentTimeMillis()/1000 - initTime < SESSION_DURATION) {
					...
					if (OUTPUT)
						out.write(peer.getRoundId() + " " + nbDeletedUpdates + "\n"); // roundId, nodeStatus, nbDeletedUpdates
				}
